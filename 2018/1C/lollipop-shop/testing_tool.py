...
for case in range(50):
  ...
  N = 200
  probs = [random.uniform(0.005, 0.1) for i in range(N)]
  case = []
  for customer in range(N):
    prefs = [i for i in range(N) if random.random() < probs[i]]
    case.append(prefs)
  CASES.append(case)
  MAX.append(0)
...
for i in range(NUM_TEST_CASES):
  ...
  for j in range(N):
    ...
    if q != -1:
      ...
      used[q] = True
      sold = sold + 1
  print("Case {}: sold {}".format(i+1, sold))
  sum_sold += sold

print("average sold {}".format(sum_sold / NUM_TEST_CASES))
